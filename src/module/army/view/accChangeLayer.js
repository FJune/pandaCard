...
var accChangeLayer = ModalDialog.extend({
    LayerName:"accChangeLayer",
    ...
    customWidget:function() {
        var wgtArr = [];
        wgtArr.push("accList");   //list列表
        wgtArr.push("backBtn");   //返回按钮
        wgtArr.push("Text_no");   //提示

        var uiChangeLayer = ccsTool.load(res.uiAccChangeLayer, wgtArr);
        //控件的名字赋值给this变量
    ...
    updateInfo:function(){
        ...
        var accArrayName = [STRINGCFG[100021].string, STRINGCFG[100022].string, STRINGCFG[100023].string, STRINGCFG[100024].string, STRINGCFG[100025].string, STRINGCFG[100026].string];
        var depotArray = [];
        ...
        if(depotArray.length != 0){
            ...
            for(var i=0;i<depotArray.length;i++){
                var wgtArr = [];
                wgtArr.push("accBackGround");   //最大的底
                wgtArr.push("equipImageBg");   //装备图片
                wgtArr.push("accName");   //装备名称
                wgtArr.push("accType");   //装备类型
                wgtArr.push("accRefineAttr");   //装备属性
                wgtArr.push("accRefineLev");   //精炼等级
                wgtArr.push("dressBtn");   //穿戴按钮
                var equItem = ccsTool.load(res.uiAccShowLayer,wgtArr);
                var wgt = {};
                //控件的名字赋值给wgt变量
                for(var key in equItem.wgt){
                    wgt[key] = equItem.wgt[key];
                }
				wgt.accBackGround.removeFromParent(false);
                this.accList.pushBackCustomItem(wgt.accBackGround);
                //装备图片
                ...
                wgt.accName.setString(itemCfg.itemname);
                Helper.setNamecolorByQuality(wgt.accName,itemCfg.quality);
                //装备类型
                var equipAttri = Helper.findEqById(depotArray[i].p);
                wgt.accType.setString("【"+accArrayName[equipAttri.type -1]+"】");
                //装备属性
                ...
                wgt.accRefineAttr.setString(strAttr);
                //精炼等级
                if(depotArray[i].r == 0){
                    wgt.accRefineLev.setVisible(false);
                }else{
                    wgt.accRefineLev.setVisible(true);
                    var strText = StringFormat(STRINGCFG[100027].string,depotArray[i].r);  //100027 精炼$1阶
                    wgt.accRefineLev.setString(strText);
                }
                //穿戴按钮
